...
case class ReplaceCTERefWithInMemoryCache(session: SparkSession) extends Rule[LogicalPlan] {
  ...
  /** Derive a readable cache name from the CTE plan's output qualifiers. */
  private def cacheName(cteDef: CTERelationDef): String = {
    val tables = cteDef.child.collectLeaves()
      .flatMap(_.output.flatMap(_.qualifier.lastOption))
      .distinct.take(3)
    if (tables.nonEmpty) s"cte[${tables.mkString(",")}]_${cteDef.id}"
    else s"cte_${cteDef.id}"
  }
  ...
  private def replaceWithCache(
    ...
    case WithCTE(child, cteDefs) =>
      ...
      // Count references per CTE in the outer plan including subquery expressions
      val refCounts = mutable.HashMap.empty[Long, Int].withDefaultValue(0)
      def countRefs(plan: LogicalPlan): Unit = plan.foreach {
        case ref: CTERelationRef => refCounts(ref.cteId) += 1
        case node =>
          node.expressions.foreach(_.foreach {
            case e: SubqueryExpression => countRefs(e.plan)
            case _ =>
          })
      }
      countRefs(child)
        ...
        if (!cteDef.deterministic || cteDef.underSubquery) {
          ...
          if (existing.isDefined) {
            ...
            cacheManager.cacheQuery(
              session, processedChild, Some(name), storageLevel)
            cacheManager.lookupCachedData(session, processedChild) match {
              case Some(cd) =>
                logInfo(s"CTE cache created: $name " +
                  s"(refs=${refCounts(cteDef.id)}, level=$storageLevel)")
                cteCache.put(cteDef.id, cd.cachedRepresentation)
              case None =>
                logInfo(s"CTE cache failed for $name")
                uncached += cteDef
            }
          } else {
            uncached += cteDef
      ...
      other
        ...
        .transformExpressionsWithPruning(_.containsPattern(CTE)) {
          case e: SubqueryExpression if e.plan.containsPattern(CTE) =>
            e.withNewPlan(replaceRefsOnly(e.plan, cteCache))
        }

    case other => other
  }

  /** Replace CTERelationRef nodes only, without processing WithCTE nodes in subqueries. */
  private def replaceRefsOnly(
      plan: LogicalPlan,
      cteCache: mutable.HashMap[Long, LogicalPlan]): LogicalPlan = plan match {
    case ref: CTERelationRef =>
      cteCache.get(ref.cteId).map { cachedPlan =>
    ...
    case other if other.containsPattern(CTE) =>
      other
        .withNewChildren(other.children.map(c => replaceRefsOnly(c, cteCache)))
        .transformExpressionsWithPruning(_.containsPattern(CTE)) {
          case e: SubqueryExpression if e.plan.containsPattern(CTE) =>
            e.withNewPlan(replaceRefsOnly(e.plan, cteCache))
        }
